...
float Guilmon::Attack(int AtkNum)
{
	float Damage = 0;
	if (AtkNum == 1) {
		Damage = Base.Damage;
...
void Guilmon::Damage(float damage)
{
...
int Guilmon::AttackUsed(int RoundsPassed)
{
	int Atk = 0;
...
bool Guilmon::Dodge(int EnemySpeed)
{
	random_device rd; // obtain a random number from hardware
	mt19937 gen(rd()); // seed the generator
