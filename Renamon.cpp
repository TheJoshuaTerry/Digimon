...
float Renamon::Attack(int AtkNum)
{
	float Damage = 0;
	if (AtkNum == 1) {
		Damage = Base.Damage;
...
void Renamon::Damage(float damage)
{
...
int Renamon::AttackUsed(int RoundsPassed)
{
	int Atk = 0;
...
// Used to dodge the incoming attacks
bool Renamon::Dodge(int EnemySpeed)
{
	random_device rd; // obtain a random number from hardware
	mt19937 gen(rd()); // seed the generator
...
// How fast an attack is used
int Renamon::Move()
{
	random_device rd; // obtain a random number from hardware
	mt19937 gen(rd()); // seed the generator
	uniform_int_distribution<> RanSpeedMultiplyer(1, 5); // define the range
	int Speed = RanSpeedMultiplyer(gen) * Info.Speed;
	return Speed;
}

void Renamon::Deleted()
{
	cout << Info.Name << " Was Deleted!!!" << endl;
}

void Renamon::Digivolve()
{

}

void Renamon::UseCard()
{
